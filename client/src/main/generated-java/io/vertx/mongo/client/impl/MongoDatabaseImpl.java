...
public class MongoDatabaseImpl extends MongoDatabaseBase {
  ...
  @Override
  public ListCollectionsResult<JsonObject> listCollections() {
    ListCollectionsPublisher<JsonObject> __publisher = wrapped.listCollections(JsonObject.class);
    return new ListCollectionsResultImpl<>(clientContext, __publisher, clientContext.getConfig().getOutputMapper());
  }

  @Override
  public ListCollectionsResult<JsonObject> listCollections(ListCollectionsOptions options) {
    ListCollectionsPublisher<JsonObject> __publisher = wrapped.listCollections(JsonObject.class);
    options.initializePublisher(clientContext, __publisher);
    Integer __batchSize = options.getBatchSize();
    if (__batchSize != null) {
      return new ListCollectionsResultImpl<>(clientContext, __publisher, clientContext.getConfig().getOutputMapper(), __batchSize);
    } else {
      return new ListCollectionsResultImpl<>(clientContext, __publisher, clientContext.getConfig().getOutputMapper());
    }
  }
  ...
  @Override
  public ListCollectionsResult<JsonObject> listCollections(ClientSession clientSession) {
    requireNonNull(clientSession, "clientSession is null");
    com.mongodb.reactivestreams.client.ClientSession __clientSession = clientSession.toDriverClass(clientContext);
  ...
  @Override
  public AggregateResult<JsonObject> aggregate(JsonArray pipeline) {
    requireNonNull(pipeline, "pipeline is null");
    List<? extends Bson> __pipeline = clientContext.getMapper().toBsonList(pipeline);
    AggregatePublisher<JsonObject> __publisher = wrapped.aggregate(__pipeline, JsonObject.class);
    return new AggregateResultImpl<>(clientContext, __publisher, clientContext.getConfig().getOutputMapper());
  }

  @Override
  public AggregateResult<JsonObject> aggregate(JsonArray pipeline, AggregateOptions options) {
    requireNonNull(pipeline, "pipeline is null");
    List<? extends Bson> __pipeline = clientContext.getMapper().toBsonList(pipeline);
    AggregatePublisher<JsonObject> __publisher = wrapped.aggregate(__pipeline, JsonObject.class);
    options.initializePublisher(clientContext, __publisher);
    Integer __batchSize = options.getBatchSize();
    if (__batchSize != null) {
      return new AggregateResultImpl<>(clientContext, __publisher, clientContext.getConfig().getOutputMapper(), __batchSize);
    } else {
      return new AggregateResultImpl<>(clientContext, __publisher, clientContext.getConfig().getOutputMapper());
    }
  }
  ...
  @Override
  public AggregateResult<JsonObject> aggregate(ClientSession clientSession, JsonArray pipeline) {
    requireNonNull(clientSession, "clientSession is null");
    requireNonNull(pipeline, "pipeline is null");
    com.mongodb.reactivestreams.client.ClientSession __clientSession = clientSession.toDriverClass(clientContext);
    List<? extends Bson> __pipeline = clientContext.getMapper().toBsonList(pipeline);
