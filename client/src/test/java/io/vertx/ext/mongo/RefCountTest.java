...
public class RefCountTest extends MongoTestBase {
  ...
  private LocalMap<String, Object> getLocalMap() {
    return vertx.sharedData().getLocalMap("__vertx.MongoClientBase.datasources");
  }
  ...
  public void testSharedDefault() throws Exception {
    LocalMap<String, Object> map = getLocalMap();
    ClientConfig config = getConfig();
    MongoClient client1 = MongoClient.createShared(vertx, config, "DEFAULT_POOL");
    assertEquals(1, map.size());
    MongoClient client2 = MongoClient.createShared(vertx, config, "DEFAULT_POOL");
    assertEquals(1, map.size());
    MongoClient client3 = MongoClient.createShared(vertx, config, "DEFAULT_POOL");
    assertEquals(1, map.size());
    client1.close();
    assertEquals(1, map.size());
    client2.close();
